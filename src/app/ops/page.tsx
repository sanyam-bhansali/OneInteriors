 ...
 *
 * ⚠️ No authentication yet. This route is gated by nothing, and it must not
 * reach production until OI-3 (WhatsApp OTP) and a role check are in place.
 * `middleware.ts` blocks it outside development as a stopgap.
 *
 * The queue is ordered by what needs a human first, not alphabetically: the
 * closer a studio is to promotion, the more valuable an hour of ops time spent
 * on it — and expired checks jump the queue because a lapsed studio is
 * currently showing a badge it no longer supports.
 */
