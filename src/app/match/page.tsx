...
export default function MatchPage() {
  ...
  const matches = useMemo(() => (hydrated ? rankStudios(brief, STUDIOS, 9) : []), [brief, hydrated]);
            ...
            {matches.length === 0 ? (
              ...
              <>
                ...
                <ul className="m-0 grid list-none grid-cols-1 gap-4 p-0 md:grid-cols-2">
                  {matches.map((m) => (
                    <StudioCard key={m.studioId} match={m} />
                  ))}
                </ul>
              </>
            )}
...
function StudioCard({ match }: { match: MatchResult }) {
  const studio = getStudioById(match.studioId);
