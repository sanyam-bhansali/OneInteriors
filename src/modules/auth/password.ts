...
/* ── The rules ────────────────────────────────────────────────────────
   ...
   These live in `password-rules.ts` and are re-exported here so every
   existing server caller and the test suite keep one import path.

   They had to move out: this file imports `node:crypto` for scrypt, and
   webpack resolves imports per MODULE rather than per binding — so the
   instant a client component imported `passwordChecks` from here, the
   whole file followed it into the browser bundle and the build failed
   with UnhandledSchemeError. The rules the screen needs had to
   physically leave the file the hashing lives in. See that file. */
export {
  MIN_PASSWORD,
  MAX_PASSWORD,
  passwordChecks,
